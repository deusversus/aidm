...
def _get_merge_analysis_prompt() -> str:
    return get_registry().get_content(
        "profile_merge_analysis",
        fallback="You are analyzing two anime profiles to prepare a merge.",
    )


def _get_merge_execution_prompt() -> str:
    return get_registry().get_content(
        "profile_merge_execution",
        fallback="You are merging two anime profiles into a hybrid profile.",
    )
...
class ProfileMergeAgent(AgenticAgent):
    ...
    def system_prompt(self) -> str:
        return _get_merge_analysis_prompt()  # all call sites use system_prompt_override; this is the correct fallback
    ...
    async def analyze(
        ...
        findings = await self.call_with_tools(
            ...
            system_prompt_override=_get_merge_analysis_prompt(),
            max_tool_rounds=8,
        )
    ...
    async def merge_with_answers(
        ...
        result_text = await self.call_with_tools(
            ...
            system_prompt_override=_get_merge_execution_prompt(),
            max_tool_rounds=6,
        )
