...
class OpeningStatePackage(BaseModel):
    ...
    # Phase 2 enrichment — stamped by compiler post-assembly, not generated by sz_handoff agent
    relationship_graph: list[RelationshipRecord] = Field(
        default_factory=list,
        description="Canonical relationships between entities, sourced from entity resolver output"
    )
    contradictions_summary: list[ContradictionRecord] = Field(
        default_factory=list,
        description="Structured contradiction records from gap analysis; blocking ones require resolution"
    )
    orphan_facts: list[FactRecord] = Field(
        default_factory=list,
        description="Fact records whose subject entity was not resolved to a canonical entity"
    )
    lore_synthesis_notes: list[str] = Field(
        default_factory=list,
        description="Hybrid/custom profile cross-setting synthesis notes for Director awareness"
    )
