...
/**
 * DNA scales — the tonal Treatment of the story being told (blueprint §4.1).
 *
 ...
 *
 * Four-layer usage (blueprint §4.2):
 *   canonical_ — the source's natural tonal fingerprint (research output)
 *   active_   — what the player chose for THIS campaign
 *   arc_override — Director's transient partial shift (PartialDNAScales)
 *   learned   — pencil marks shade the Renderer; never mutate these values
 *
 * Boundary rule (§4.3): if changing a value changes how a given scene is
 * rendered at sentence level → Treatment (here); if it changes what happens
 * next arc → Framing (composition.ts). Three axes carry explicit precedence
 * notes below; Framing is always authoritative for structure.
 *
...
export const DNAScales = z.object({
  ...
  pacing: axis(),
  /**
   * standalone episodes (0) ↔ serialized arc (10). How much each turn commits
   * to the long thread.
   *
   * §4.3 boundary: thematic feel, not structural commitment. Framing is
   * authoritative for structure — `status_quo_stability`/`arc_shape` govern
   * what the Director plans; this axis governs only how much the prose
   * REFERENCES the long thread at sentence level.
   */
  continuity: axis(),
  ...
  /**
   * intimate (0) ↔ cosmic (10). Stakes scale — Mushishi single-village vs.
   * Gurren Lagann universe.
   *
   * §4.3 boundary: thematic feel, not structural commitment — Mushishi can
   * FEEL cosmic in scope while structurally staying single-village. Governs
   * the felt stakes inside a scene; Framing owns actual story structure.
   */
  scope: axis(),
  /**
   * fatalist (0) ↔ agency-driven (10). Do characters drive events, or are
   * they subject to them?
   *
   * §4.3 boundary: governs the felt volition inside a scene (fatalism as
   * texture). Framing (`player_role`, `choice_weight`) is authoritative for
   * structural agency — what choices actually change.
   */
  agency: axis(),
 ...
 * on that axis. Feeds the Sakkan's drift math (active − observed) and the studio
 * view's intended-divergence display (canonical vs active).
 */
