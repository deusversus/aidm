...
export async function searchIdentity(rawTitle: string): Promise<IdentityRescue> {
  const search = await callSearch(RESEARCH_SELECTION, {
    ...
    prompt: [
      ...
      "MANDATORY: run AT LEAST ONE web search before you answer, including —",
      "especially — when the name looks invented, garbled, or unfamiliar. A",
      "verdict that no such work exists is only worth reporting when the live",
      "web was actually consulted and came back empty; concluding it from",
      "memory alone is a guess wearing a refusal, and this desk does not ship",
      "guesses. Search first, then report what the searches did or did not find.",
