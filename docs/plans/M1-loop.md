...
8. **Two pull-forwards from later milestones — RATIFIED by the user 2026-07-07** (both logged in blueprint §15): (a) **recap v1 + yokoku** ship thin at M1 (recap: compacted beats + arc state, narration-tier; yokoku: vibe-promise from the arc plan) — **the long-term staging is unchanged**: M3 still owns recap hardening exactly as §12 writes it; M1's versions are the §9.4 lifecycle's minimum, nothing more. (b) The pilot-as-normal-turn deviation (structural call 2) is likewise ratified: "one path beats two." (c) Nothing else — the draft's "studio ledger page" was cut for the opposite reason (§9.1 stages studio surfaces at M4; the soak report and `model_calls` answer the cost question at M1).
