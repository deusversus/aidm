...
**Model tiers are player-facing menus, not fixed assignments** (resolved 2026-07-06; menus amended 2026-07-25, user-directed: Opus 5 replaced Opus 4.8 on every rung — identical $5/$25 pricing, same 128k output): **narration** (Sonnet 5 / Opus 5 / Fable 5), **judgment** (Haiku 4.5 / Sonnet 5 / Opus 5), **probe** (Haiku 4.5 / Sonnet 5). Rationale: API cost control is real, users consistently select smarter models than tasks strictly need, and since reasoning effort is scaled silently by the engine, model selection is the closest thing the player has to an AI-tier throttle. Selection is per campaign per tier, **changeable at any time** — a player is never stuck above their budget or below their payday — with a **model-change warning** covering cache-reset cost and possible voice shift (the "studio handoff" disclosure). Effort↔contract mapping — **amended 2026-08-01 (user-ruled): FLAT `high` across all tiers.** Effort participates in the prompt-cache key (wire-measured 2026-07-26); the original ladder (trivial→`low`, heavy→`xhigh`) bought a cold ~25k-token prefix re-write at every tier boundary, and the 2026-07/08 playtest measured genga-at-`high` fully warm ($0.38/turn) while sakuga-at-`xhigh` opened cold every time. `high` is the API default and thinks deeply; `xhigh`'s quality delta was designed-in, never measured — it returns only via a blind A/B (two sakuga scenes, both efforts, read cold). The original caveat stands vacuously satisfied: every beat now runs ≥`high`. The Director's session review runs on the campaign's narration model (the showrunner is a creative mind). Fable-narration campaigns ship with server-side fallback to Opus 5 configured (Opus 4.8 until 2026-07-25); any fallback event lands in the trace as Gauge-relevant.
...
- Content: ~600–900 tokens of prose pressure. **[AMENDED 2026-08-05, user-ruled: band is now 1,200–2,600 — the M2R4 voice work (author's hand, §4.5 checklist, exemplars) legitimately outgrew the original target (measured 2,124 after 11 trims); the user widened the proposed 2,200 ceiling ("razor thin"). Cached, ~$0.001/turn. Trim ladder inverted the same day: voice matter trims LAST, author sample never.]** Composition: identity paragraph → up to **6 rendered axes** (extremes only: ≤3 or ≥7, ranked by |distance from 5| and arc relevance) each as craft instruction + at most **2–3 exemplar passages total** (§4.7) for the most extreme axes → one summarizing sentence per non-extreme axis group (7 groups) → Voice fingerprint verbatim → standing pencil-mark craft notes (§6.6).
...
**Tier vocabulary (register):** trivial turns are **douga** (in-betweens), standard turns are **genga** (key frames), heavy at full budget is **sakuga**. Effort mapping: FLAT `high` across all tiers (§3 amendment, user-ruled 2026-08-01 — effort is part of the prompt-cache key; the original ladder and its promotion caveat are recorded there). **Douga-routing** — sending in-between turns to a cheaper narration model, as real studios outsource in-betweens — is a parked M6 experiment: authentic to the medium, but deliberate voice variance; the Sakkan watches before anyone ships it.
...
- **Breakpoints:** one at Block 1 tail, one at Block 2 tail, one at Block 3 tail (refreshed each turn). **Amended 2026-07-26 (user-signed, M2R5 C3):** Block 3 renders as *discrete* blocks — pin head, then one block per exchange — with breakpoints at the pin head (when pins exist) and on the **last exchange block, moving each turn**; Blocks 1–2 keep theirs — up to the API's four, three in the common no-pins shape. Rationale: as one growing block, B3 re-wrote a 12k-token median window at the 2× 1h rate every turn and nothing read it back (measured, net-negative cache); with a moving tail the prior window reads at 0.1× and only the new exchange writes (wire-measured at landing: turn N+1 read exactly turn N's read+write, wrote 47 tokens). Append-only is untouched — same rows, same watermark derivation; the concatenated prompt is byte-identical for every non-empty window (test-pinned), with ONE deliberate deviation: an empty window (turn 1) no longer renders the orphan 'Recent play' header over nothing.
...
1. **Model tiers = player-facing menus** — see §3. Narration: Sonnet 5 / Opus 5 / Fable 5 · Judgment: Haiku 4.5 / Sonnet 5 / Opus 5 · Probe: Haiku 4.5 / Sonnet 5 (Opus 5 replaced Opus 4.8 per the player, 2026-07-25). Changeable anytime with cache-reset + voice-shift warning.
