...
- **The push gate is the FULL remote gate**: `pnpm lint && pnpm typecheck && pnpm test && pnpm evals:ci` — CI runs these same gates (plus a db:migrate setup step), and `evals:ci` is NOT part of `pnpm test`. **`evals:ci` is deterministic-only** (`--ci` excludes every `requiresLlm` suite — the canary, channel-routing, control-key, thin-ip-drill never run in it). **Any push touching a live-play prompt or a production schema ALSO runs the live sweep first: bare `pnpm evals` (~$0.90/run, user-ratified 2026-08-05)** — two real regressions (a buried agency clause; a routing gate) sat invisible for days because only the deterministic gate ever ran. After pushing, confirm the remote run went green (`gh run list --limit 1`). Gate commands run BARE or `&&`-chained on their own exit codes — piping a gate through grep/tail in the same chain as commit/push swallows the failure (it happened: a red vitest run pushed on 2026-07-11; the failure was flaky, the hole was real). (Learned 2026-07-11: CI was red for seven pushes because evals:ci never ran locally.)
