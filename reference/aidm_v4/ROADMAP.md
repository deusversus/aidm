...
**Known debt (v3 parity audit 2026-04-21) — CLOSED 2026-04-21.** All 7 phases from `docs/plans/v3-audit-closure.md` shipped. 5 BLOCKERs closed (override persistence, WB-accepted entity persistence, composition mode-shift, vocab-freshness whitelists, retrieval-tier alignment); rule library + context blocks landed as their own Phase 2/3 substacks (156 rule-library entries, context_blocks table + generator + Block 2 wiring). Phase 4 memory governance (decay curves + flags + boost-on-access); Phase 5 meta conversation loop; Phase 6 transient NPCs + WB reshape to editor; Phase 7 polish (BEAT_CRAFT, style-drift persistence, LIMITATIONS bolded, multi-query interface, prompt-eval harness, prompt dep-graph tool). 420 tests. Migrations 0004-0008 applied. Phase 8 items (semantic retrieval runtime, portrait resolver, research-phase eval) deferred to M4 per explicit plan. See `docs/plans/M1-closure.md` "Known debt" + commit stack.
